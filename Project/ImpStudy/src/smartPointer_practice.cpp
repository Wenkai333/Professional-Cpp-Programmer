...
public:
    Entity(const std::string& name, int health) : name_(name), health_(health) {
...
public:
    Player(const std::string& name) : Entity(name, 100), score_(0) {
        std::cout << "  👤 Player '" << name_ << "' joined the game!\n";
    }

    ~Player() override {
        std::cout << "  👋 Player '" << name_ << "' left the game (Score: " << score_ << ")\n";
    }

    void addScore(int points) {
        score_ += points;
        std::cout << "  ⭐ " << name_ << " scored " << points << " points! (Total: " << score_
                  << ")\n";
    }
...
public:
    Enemy(const std::string& name, int health, int damage) : Entity(name, health), damage_(damage) {
...
std::unique_ptr<Enemy> createEnemy(const std::string& name, int health, int damage) {
    ...
    return std::make_unique<Enemy>(name, health, damage);
}
...
public:
    Item(const std::string& name, int value) : name_(name), value_(value) {
        std::cout << "  📦 Item '" << name_ << "' created (Value: " << value_ << ")\n";
    }
...
public:
    ...
    void addItem(std::shared_ptr<Item> item) {
        ...
        std::cout << "  ➕ Added [item_name] to inventory" << std::endl;
    }
    ...
    bool removeItem(const std::string& name) {
        ...
        auto found =
            std::find_if(items_.begin(), items_.end(),
                         [&name](const std::shared_ptr<Item>& a) { return a->getName() == name; });
        if (found != items_.end()) {
            items_.erase(found);
            std::cout << "  ➖ Removed [ " << name << " ] from inventory" << "\n";
            return true;
        }
    ...
    int getTotalValue() const {
        ...
        auto values =
            items_ | std::views::transform([](const auto& item) { return item->getValue(); });
...
std::shared_ptr<Item> createSharedItem(const std::string& name, int value) {
    ...
    return std::make_shared<Item>(name, value);
}
...
public:
    ...
    bool hasTeam() const {
        ...
        return !team_.expired();  // Replace this
    }
...
public:
    ...
    void addMember(std::shared_ptr<TeamMember> member) {
        ...
        if (member) {
            ...
            std::cout << "  ➕ " << member->getName() << " joined team " << name_ << "\n";
        }
    }
    ...
    bool removeMember(const std::string& name) {
        ...
        auto remove = std::find_if(
            ...
            [&name](const std::shared_ptr<TeamMember>& a) { return a->getName() == name; });
        if (remove != members_.end()) {
            std::cout << "  ➖ " << name << " left team " << name_ << "\n";
            members_.erase(remove);
            return true;
        }
    ...
    size_t getMemberCount() const {
        // Your implementation here
        return members_.size();  // Replace this
    }

    void listMembers() const {
        std::cout << "  👥 Team '" << name_ << "' Members:\n";
        for (const auto& member : members_) {
            std::cout << "    - " << member->getName() << " (Ref Count: " << member.use_count()
                      << ")\n";
        }
    }
};

inline std::string TeamMember::getTeamName() const {
    // Lock the weak_ptr to get a shared_ptr
    auto t = team_.lock();  // ✅ Store the result!

    // If valid (not expired), return team name
    if (t) {
        return t->getName();  // ✅ Use the locked shared_ptr
    }

    // If expired, return "No Team"
    return "No Team";
}
...
public:
    Texture(const std::string& filename, int width, int height)
        ...
        std::cout << "  🖼️ Texture '" << filename_ << "' loaded (" << width_ << "x" << height_
                  << ")\n";
    }
...
public:
    ...
    std::shared_ptr<Texture> load(const std::string& filename, int width, int height) {
        ...
        if (cache_.find(filename) != cache_.end()) {
            if (auto data = cache_[filename].lock()) {
                std::cout << "  ✅ Cache HIT: " << filename << "\n" << std::endl;
                return data;
            }
        }
        std::cout << "  ❌ Cache MISS: [filename]" << std::endl;
        auto shared_ptr = std::make_shared<Texture>(filename, width, height);
        cache_.emplace(make_pair(filename, shared_ptr));
        return shared_ptr;
    }
    ...
    void clearExpired() {
        ...
        std::erase_if(cache_, [](const auto& pair) { return pair.second.expired(); });
    }
    ...
    void printStats() const {
        ...
        auto validity =
            cache_ | std::views::values  // Get just the weak_ptrs
            | std::views::transform([](const auto& weak_ptr) { return !weak_ptr.expired(); });

        // Count the true values
        auto valid = std::ranges::count(validity, true);

        std::cout << "  📊 Cache: " << cache_.size() << " entries (" << valid << " valid, "
                  << (cache_.size() - valid) << " expired)\n";
    }
};
